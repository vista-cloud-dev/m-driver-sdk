...
Last reconciled: **2026-06-04** (m-ydb M2 `sync` complete — read + write).
...
| **m-ydb** | D2 — YottaDB driver | `m-ydb` | `main` | **v0.2.0** | _M2 sync done_ | `m-test-engine` (YottaDB r2.07) ☑ |
...
NEXT: **M3 exec** (`yottadb -run`/`%XCMD`/`-direct`, `$ZSTATUS`→engineError), then M5 cover
(`view "TRACE"`→LCOV). Deferred: `lifecycle logs` (parity with m-iris). Granular tasks: plan §4.
