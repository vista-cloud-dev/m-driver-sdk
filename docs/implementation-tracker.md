...
Last reconciled: **2026-06-04** (m-ydb M2 `sync` complete; M3 `exec` run/eval + engineError landed).
...
| **m-ydb** | D2 — YottaDB driver | `m-ydb` | `main` | **v0.2.0** | _M3 exec ◐ (run/eval)_ | `m-test-engine` (YottaDB r2.07) ☑ |
...
| **M3** | `exec` (load/run/eval/abort) + `engineError` | ☑ `ExecRequest`/`ExecResult`, `EngineError`, `LoadRequest`/`LoadResult` | ◐ run/eval + engineError ☑ ($ETRAP→$ZSTATUS via `%XCMD`, real r2.07); load/abort + `--prefix` NEXT | ◐ remote substrate spike ☑ (real IRIS); wire exec/eval/abort commands |
...
**M3 exec ◐**: `run`/`eval` + `engineError` done (plan §4 tasks 9-partial/10) — `ExecTrapped` xecutes
the work under a `$ETRAP` via `%XCMD` (a clean non-direct context; `-direct` prints `YDB>` prompts and
bypasses the trap), capturing the sentinel-delimited `$ZSTATUS`; `parseZStatus` → §7 EngineError
(mnemonic by `%FAC-S-NAME` regex, so a `%`-routine location isn't mis-read). Validated vs real r2.07
(`%YDB-E-LVUNDEF` round-trip). caps `exec:[run,eval]`. NEXT: **load** (stage `--from` + auto-compile;
needs routines-env for docker exec) + **abort** + ephemeral `--prefix` (plan §4 tasks 8,9). Then M5
cover (`view "TRACE"`→LCOV). Deferred: `lifecycle logs` (parity with m-iris). Granular tasks: plan §4.
